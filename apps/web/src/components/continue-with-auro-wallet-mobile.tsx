...
const paramsURL = encodeURIComponent(SITE_URL);
const endURL = `https://www.aurowallet.com/applinks?action=openurl&url=${paramsURL}`;

export function ContinueWithAuroWalletMobileModal(props: DialogProps) {
	return (
		<Dialog modal {...props}>
			<DialogContent className="bg-white text-zinc-800 sm:max-w-[425px]">
				<DialogHeader>
					<DialogTitle className="text-2xl font-bold text-zinc-700">
						Continue with <span className="text-primary">Auro Wallet</span>
					</DialogTitle>
					<DialogDescription className="text-zinc-700">
						We have detected that you are using a mobile device. To interact
						with this site, you need to continue with Auro Wallet app.
					</DialogDescription>
				</DialogHeader>
				<div className="pb-4">
				...
				<DialogFooter className="flex flex-col gap-2 sm:flex-row sm:justify-between">
					...
					<Button className="w-full sm:w-auto" asChild>
						<Link href={endURL} target="_blank">
							Open Auro Wallet
